...
class VoiceBox(nn.Module):
    def __init__(
        ...
        sigma = cfg.sigma if cfg is not None and hasattr(cfg, "sigma") else float(sigma)
        cond_scale_factor = (
        ...
        repa_layer_idx = (
            cfg.repa_layer_idx
            if cfg is not None and hasattr(cfg, "repa_layer_idx")
            else repa_layer_idx
        )
        ...
        self.diff_estimator = DiffLlama(
            ...
            repa_layer_idx=self.repa_layer_idx,
        )
    ...
    def loss_t_with_hidden_features(
        self,
        x,
        x_mask,
        t,
        ...
        """
        Same as loss_t but also returns hidden features from the diffusion estimator.
        """
        xt, z, new_t, prompt_len, mask = self.forward_diffusion(x, t)

        noise = z

        # drop all condition for cfg with probability cfg_scale
        if cond is not None:
            cond = cond * torch.where(
                prompt_len > 0,
                torch.ones_like(prompt_len),
        ...
        # Get flow prediction and hidden features from diffusion estimator
        flow_pred, hidden_features = self.diff_estimator.forward_with_hidden_features(xt, new_t, cond, x_mask)

        # final mask used for loss calculation
        final_mask = mask * x_mask[..., None]  # (B, T, 1)

        return noise, x, flow_pred, final_mask, prompt_len, hidden_features
    ...
    def forward(self, x, x_mask, cond_code=None, cond_feature=None):
        ...
        assert abs(cond_feature.shape[1] - x.shape[1]) <= 5, "cond_feature and x should have the same length"
    ...
    def forward_with_hidden_features(self, x, x_mask, cond_code=None, cond_feature=None):
        """
        Forward pass that also returns hidden features for REPA loss computation.
        x.shape: (B, T, mel_dim)
        x_mask.shape: (B, T)
        cond_feature.shape: (B, T, cond_dim)
        """
        if cond_code != None and self.use_cond_code:
            cond_feature = self.cond_emb(cond_code)
        if self.cond_scale_factor != 1:
        ...
        min_len = min(cond_feature.shape[1], x.shape[1])
        assert abs(cond_feature.shape[1] - x.shape[1]) <= 8, "cond_feature and x should have the same length"
        assert x_mask.shape[1] == x.shape[1], "x_mask and x should have the same length"
        x, x_mask, cond_feature = (
            x[:, :min_len],
            x_mask[:, :min_len],
        ...
        # Use the method that returns hidden features
        t = torch.rand(x.shape[0], device=x.device, requires_grad=False)
        t = torch.clamp(t, 1e-5, 1.0)
        if self.time_scheduler == "cos":
            t = 1 - torch.cos(t * math.pi * 0.5)
        
        noise, x, flow_pred, final_mask, prompt_len, hidden_features = self.loss_t_with_hidden_features(
            x, x_mask, t, cond_feature
        )
        return noise, x, flow_pred, final_mask, prompt_len, hidden_features
...
# Global cache for mel spectrogram models
_mel_model_cache = {}
...
def extract_normalized_mel_spec_50hz(speech, device="cuda"):
    ...
    # Check if we already have a cached model for this device
    if device not in _mel_model_cache:
        mel_model = MelSpectrogram(
            sampling_rate=24000,
            n_fft=1920,
            num_mels=128,
        ...
        mel_model.eval()
        _mel_model_cache[device] = mel_model
    else:
        mel_model = _mel_model_cache[device]
