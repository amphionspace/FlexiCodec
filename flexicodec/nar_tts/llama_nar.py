...
class DiffLlama(LlamaModel):
    ...
    def forward(
        ...
        condition_length = cond_embedding.shape[1]

        if self.conditioning_type == "add":
            x = x + cond_embedding
        elif self.conditioning_type == "concat":
            x = torch.cat([cond_embedding, x], dim=-1)
        else:
        ...
        if self.conditioning_type == "concat":
            hidden_states = hidden_states[:, :, :condition_length]

        hidden_states = self.mel_out_mlp(hidden_states)

        return hidden_states

    def forward_with_hidden_features(
        self,
        x,
        diffusion_step,
        cond,
        ...
        for idx, decoder_layer in enumerate(self.layers):
            ...
            hidden_states = layer_outputs[0]
            if idx == self.repa_layer_idx:
                repa_features = hidden_states
        ...
        return hidden_states, repa_features
